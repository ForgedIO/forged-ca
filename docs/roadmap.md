...
- [x] **Slice 1.8** — left sidebar IA + dedicated pages per menu item (replaces the original single-page navigation; sets up the surface every later slice plugs pages into)
- [x] **Slice 2** — step-ca daemon lifecycle under Settings (**2A**), ACME provisioner model + admin page + ca.json wiring (**2B**), ACME Client Onboarding page with tabbed per-client snippets — `step`, `certbot`, `acme.sh`, cert-manager, Traefik, Caddy, nginx (**2C**)
- [x] **Slice 3** — cert templates: CRUD UI, template-per-provisioner binding, default "Web Server (Server + optional Client Auth)" template. Template signer policy: ForgedCA-only (no external-CA delegation; direct external-CA requests to ACME or Slice 3.5 SCEP instead).
- [ ] **Slice 3.5** — SCEP provisioner + onboarding page (peer to ACME, for network equipment and MDM-managed devices): enable step-ca's built-in SCEP provisioner, admin UI to configure (challenge type, template binding), onboarding snippets for Cisco IOS / ASA, FortiGate, Palo Alto, Aruba, and the generic MDM enrollment URL (Intune / JAMF). Same audit + dashboard surface as ACME — ForgedCA remains the only signer.
- [x] **Slice 3.7** — template policy enrichment: Extended Key Usage allowlist on each template (friendly checkboxes — Server authentication, Client authentication, Code signing, Email / S/MIME, Time stamping, OCSP signing, Smart card logon, IPsec — plus an "advanced: custom OID" escape hatch), Key Usage bits as the natural pair (defaults derived from chosen EKUs so admins don't have to think unless they want to), and a migration that upgrades the seeded "Web Server" template to EKU = serverAuth + clientAuth and KU = digitalSignature + keyEncipherment. **Shipped enforcement is pinning, not rejection:** each ACME provisioner emits `options.x509.template` rendered from its bound template, so step-ca writes exactly the declared EKU/KU on every issued cert regardless of what the CSR asks for — a misbehaving client can't smuggle `codeSigning` into a web-server template. Subject DN policy and SAN name-constraints deferred to a later slice (multi-tenant scenarios, less urgent for v1).
- [ ] **Slice 4** — non-ACME CSR signing UI (**full spec: [`docs/slice-4-spec.md`](slice-4-spec.md)**): admin uploads CSR → picks a compatible template → ForgedCA validates the CSR's requested EKU/KU against the Slice 3.7 allowlist and rejects out-of-allowlist requests with a clear reason (a manual upload flow can tell the admin *why*, where ACME can only pin) → signs; explicit "passthrough (advanced)" opt-in bypasses template policy for one-off PKI-expert work (still ForgedCA-signed and audited)
...
- **Endpoint cert lifecycle for non-ACME devices** (Keyfactor Command-style): polling agent + per-device drivers (F5 BIG-IP, Palo Alto, IIS/WinRM, generic SSH+hook), TLS endpoint discovery scan, ownership/expiry/weak-crypto dashboard. Strong candidate for a future paid SaaS tier on AWS Marketplace — ForgedCA OSS core remains internal-PKI issuance only. See `docs/keyfactor-research.md` for full capability analysis.
