...
- [ ] **Slice 3.7** — template policy enrichment: Extended Key Usage allowlist on each template (friendly checkboxes — Server authentication, Client authentication, Code signing, Email / S/MIME, Time stamping, OCSP signing, Smart card logon, IPsec — plus an "advanced: custom OID" escape hatch), Key Usage bits as the natural pair (defaults derived from chosen EKUs so admins don't have to think unless they want to), and a migration that upgrades the seeded "Web Server" template to EKU = serverAuth + clientAuth and KU = digitalSignature + keyEncipherment. Issuance validates the CSR's requested EKU/KU against the template's allowlist — out-of-allowlist requests rejected with a clear reason. Subject DN policy and SAN name-constraints deferred to a later slice (multi-tenant scenarios, less urgent for v1).
- [ ] **Slice 4** — non-ACME CSR signing UI: admin uploads CSR → picks a compatible template → ForgedCA validates the CSR's requested EKU/KU against the template's allowlist (per Slice 3.7) → signs; explicit "passthrough (advanced)" opt-in bypasses template policy for one-off PKI-expert work (still ForgedCA-signed and audited)
