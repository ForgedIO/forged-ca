...
- [ ] **Slice 4** — non-ACME CSR signing UI: admin uploads CSR → picks a compatible template → ForgedCA validates the CSR's requested EKU/KU against the Slice 3.7 allowlist and rejects out-of-allowlist requests with a clear reason (a manual upload flow can tell the admin *why*, where ACME can only pin) → signs; explicit "passthrough (advanced)" opt-in bypasses template policy for one-off PKI-expert work (still ForgedCA-signed and audited)
